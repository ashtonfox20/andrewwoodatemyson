    ...
    extends StochasticAgent
{
    ...
    public void valueIteration(StateView state) {
        ...
        Map<Coordinate, Double> toFill = getZeroMap(state);
        Map<Coordinate, Double> utilitiesGrid = new HashMap<>(toFill);
        // this.setUtilities(getZeroMap(state)); //old attempt that was janky
        double diff;
        do{
            diff = 0.0;
            toFill = new HashMap<>(utilitiesGrid);
            for(Coordinate coordinate : toFill.keySet()){
                if(!isTerminalState(coordinate)){
                    double bestUtility = Double.NEGATIVE_INFINITY;
                    for (Direction direction : TransitionModel.CARDINAL_DIRECTIONS){
                        double thisMove = 0.0;
                        for(Pair<Coordinate, Double> transition : TransitionModel.getTransitionProbs(state, coordinate, direction)){
                            double nextStateUtility = toFill.get(transition.getFirst());
                            thisMove += transition.getSecond() * nextStateUtility;
                        }
                        if(thisMove > bestUtility){
                            bestUtility = thisMove;
                        }
                    }
                    double newUtility = RewardFunction.getReward(coordinate) + GAMMA * bestUtility;
                    utilitiesGrid.put(coordinate, newUtility);
                    diff = Math.max(diff, Math.abs(newUtility - toFill.get(coordinate)));
                }else{
                    utilitiesGrid.put(coordinate, RewardFunction.getReward(coordinate));
                }
            }
        }while(diff > EPSILON *(1-GAMMA)/GAMMA);
        setUtilities(toFill);
    }
